...
internal sealed class FakePredictor(Func<string, Questions, OrderedDictionary<string, Answer>> answer) : ILayaPredictor
{
    ...
    public LayaResult Predict(LayaState state, Questions questions, PredictOptions? options = null)
    {
        ...
        return new LayaResult(LayaAgent.ResultModelName, answer(state.Serialize(), questions), new Usage(1));
    }
    ...
    public static NoulAnswer Noul(double p) => new() { Noul = p, Confidence = Math.Max(p, 1 - p), AnswerConfidence = Math.Max(p, 1 - p) };

    public static ScoreAnswer Score(double score) =>
        new() { Score = score, Legend = [], Probabilities = new() { ["0"] = 1.0 }, Confidence = 0.5, AnswerConfidence = 0.5 };
